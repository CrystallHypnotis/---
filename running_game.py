...
def running_game(fps, difficulty, width, height, screen):
    if difficulty == "picts/normal_menu_proj.png":
        difficulty = 1
    elif difficulty == "picts/hard_menu_proj.png":
        difficulty = 2
    elif difficulty == "picts/pain_menu_proj.png":
        difficulty = 3
    count = 0
    screen_count = 0
    health = 10
    ...
    class Player(pygame.sprite.Sprite):
        sprite = pygame.image.load("picts/player_pyg.png")
    ...
    class Bullet(pygame.sprite.Sprite):
        sprite = pygame.image.load("picts/bullet_pyg.png")
    ...
    class EnemyBullet(pygame.sprite.Sprite):
        sprite = pygame.image.load("picts/bullet_enemys.png")
        ...
        def __init__(self, *group, pos_enemy, pos_player):
            ...
            self.total_speed = 4
            self.size = (32, 32)
            self.image = pygame.transform.scale(EnemyBullet.sprite, self.size)
    ...
    class EnemyOne(pygame.sprite.Sprite):
        sprite = pygame.image.load("picts/enemy_one_pygame_proj.png")
        ...
        def __init__(self, *group, pos_spawn, hp=hp_of_enemies):
            ...
            self.size = (32, 32)
            self.hp = hp
            self.image = pygame.transform.scale(EnemyOne.sprite, self.size)
    ...
    class EnemyTwoShooter(pygame.sprite.Sprite):
        sprite = pygame.image.load("picts/enemy_two_pygame_proj.png")
        sprite_1 = pygame.image.load('picts/enemy_one_pygame_proj.png')
        size = (32, 32)
        ...
        def __init__(self, *group, pos_of_spawn, max_x, max_y, hp):
            ...
            self.total_speed = 2
            self.size = (32, 32)
            self.image = pygame.transform.scale(EnemyTwoShooter.sprite, self.size)
        ...
        def update(self, pos_of_player, *args):
            if args and args[0].type == enemy_is_shooting:
                EnemyBullet(enemy_bullets, pos_enemy=self.rect.center, pos_player=pos_of_player)
    ...
    class EnemyThreeDozer(pygame.sprite.Sprite):
        sprite = pygame.image.load("picts/buldozer_full_armor.png")
        sprite_1 = pygame.image.load('picts/buldozer_2.3_armor.png')
        sprite_2 = pygame.image.load('picts/buldozer_1.3_armor.png')
        sprite_3 = pygame.image.load('picts/buldozer_0.3_armor.png')
        size = (32, 32)
        count = 0
        ...
        def __init__(self, *group, pos_of_spawn, max_x, max_y, hp):
            ...
            self.total_speed = 1
            self.size = (128, 128)
            self.image = pygame.transform.scale(EnemyThreeDozer.sprite, self.size)
    ...
    Player(player, max_x=info_object.current_w, max_y=info_object.current_h)
    ...
    Title(titles, pos=(width // 12 - 50, height // 12), slides_and_sizes=[["picts/score_t_proj.png", (123, 30)]])

    temp_score_hud = Hud(text=score, size=45, color=(255, 255, 255), screen=screen,
                         pos=(width // 12 + 30, height // 12 - 32))

    while running:
        # EnemyOne(enemies, pos_spawn=[random.randint(0, width), random.randint(0, height)])
        clock.tick(fps)
        player.update()
        bullets.update()
        ...
        if screen_count >= needed:
            upp = level_up(screen_main, info_object.current_w, info_object.current_h, fps)
            if upp == 'health':
                health += 10
            elif upp == 'speed':
                pass
        ...
        for event in pygame.event.get():
            if eternal_phase:
                if event.type == spawn_dozer_2:
                    EnemyThreeDozer(enemie_dozers, pos_of_spawn=[random.randint(0, width), random.randint(0, height)],
                                    max_x=info_object.current_w, max_y=info_object.current_h, hp=hp_of_enemies * 100)
                if event.type == spawn_enemy2_3:
                    EnemyTwoShooter(enemie_shooters,
                                    pos_of_spawn=[random.randint(0, width), random.randint(0, height)],
                                    max_x=info_object.current_w, max_y=info_object.current_h, hp=hp_of_enemies)
            ...
            for enemie_shooter in enemie_shooters:
                if bullet.rect.colliderect(enemie_shooter.rect):
                    ...
                    if 0 < enemie_shooter.hp <= hp_of_enemies / 2:
                        enemie_shooter.image = pygame.transform.scale(EnemyTwoShooter.sprite_1, enemie_shooter.size)
            ...
            for enemie_dozer in enemie_dozers:
                if bullet.rect.colliderect(enemie_dozer.rect):
                    ...
                    if hp_of_enemies * 50 / 4 * 2 < enemie_dozer.hp <= hp_of_enemies * 50 / 4 * 3:
                        enemie_dozer.image = pygame.transform.scale(EnemyThreeDozer.sprite_1, enemie_dozer.size)
                    elif hp_of_enemies * 50 / 4 * 1 < enemie_dozer.hp <= hp_of_enemies * 50 / 4 * 2:
                        enemie_dozer.image = pygame.transform.scale(EnemyThreeDozer.sprite_2, enemie_dozer.size)
                    elif 0 < enemie_dozer.hp <= hp_of_enemies * 50 / 4 * 1:
                        enemie_dozer.image = pygame.transform.scale(EnemyThreeDozer.sprite_3, enemie_dozer.size)
        ...
        for enemy_bullet in enemy_bullets:
            ...
            for p in player:
                if p.rect.colliderect(enemy_bullet.rect):
                    ...
                    if health <= 0:
                        running = False
                        write_rec_to_db(count, 0)
                        death_screen(info_object.current_w, info_object.current_h, fps, screen_main, count)
